...
- Keep product/customer app work in `/workspace/vercel-nextjs-monorepo`, durable corporate memory in `/workspace/llm-wiki`, and this Hermes Render deployment/profile code in `/workspace/hermes-render` when those workspaces are available. The old gateway repo at `/workspace/hermes-agent-gateway-slack` is legacy context only.
